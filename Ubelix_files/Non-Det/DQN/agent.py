...
class DQNAgent:
    def __init__(self, observation_space, action_space, cfg: Config, device):
        ...
        # networks
        self.model       = QNetwork(c, h, w, self.cfg.conv_units , self.cfg.dense_units, n_actions).to(self.device)
        self.target_net  = QNetwork(c, h, w, self.cfg.conv_units , self.cfg.dense_units, n_actions).to(self.device)
